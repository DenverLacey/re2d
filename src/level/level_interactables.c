...
static void draw_item_name_above_player(Interactable *item, Vector2 item_position, float player_y) {
    char item_name[32];
    ...
    float y = player_y - PLAYER_HEIGHT - ITEM_NAME_DISPLAY_POSITION_OFFSET_Y - ITEM_NAME_DISPLAY_FONT_SIZE;
...
void level_interactables_draw(Level_Interactables *level, Vector2 player_position, Vector2 mouse_world_position) {
    for (size_t i = 0; i < level->num_objects; ++i) {
        ...
        if (CheckCollisionPointRec(mouse_world_position, item_rect) && within_pickup_distance) {
            draw_item_name_above_player(&object->interactable, object->position, player_position.y);
        }
    }
}
